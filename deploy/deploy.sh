...
./configure --prefix=$OPENSHIFT_DATA_DIR --with-pcre=$OPENSHIFT_TMP_DIR/nginx_built/pcre-8.37 --with-zlib=$OPENSHIFT_TMP_DIR/nginx_built/zlib-1.2.8  --with-ipv6 --with-http_stub_status_module --with-http_gzip_static_module --with-http_sub_module --with-http_ssl_module

#--with-openssl=/tmp/nginx_built/openssl-1.0.2
#--with-sha1=/tmp/nginx_built/sha1-0.2

if [ $? -ne 0 ]; then
	echo -e "\nbuild nginx: An error occurred. './configure...'\n"
	exit 1
fi

make
if [ $? -ne 0 ]; then
	echo -e "\nbuild nginx: An error occurred. 'make...'\n"
	exit 1
fi

make install
if [ $? -ne 0 ]; then
	echo -e "\nbuild nginx: An error occurred. 'make install...'\n"
	exit 1
fi
...
# backup - nginx.conf
cp -a $OPENSHIFT_DATA_DIR/conf/nginx.conf $OPENSHIFT_DATA_DIR/conf/nginx.conf.bak
